...
const updateCodeDiv = (result, elemId) => document.getElementById(elemId).innerHTML = Object.entries(result).map((element) => `<li>${element[0]}:${element[1]}</li>`).join("")

const updateProc = (result) => {
    document.getElementById("proc_num_attr").innerText = result["num_attractions"];
    ...
    document.getElementById("proc_last_updated").innerText = result["last_updated"];
}
...
const updateAttr = (result) => {
    ...
    document.getElementById("attr_time").innerText = result["attraction_timestamp"];
...
const getStats = () => {
    ...
    makeReq(ANALYZER_API_URL.stats, (max) => makeReqParam(ANALYZER_API_URL.attr_info, max, (result) => updateAttr(result)))
    makeReq(ANALYZER_API_URL.stats, (max) => makeReqParam(ANALYZER_API_URL.exp_info, max, (result) => updateExp(result)))
}
